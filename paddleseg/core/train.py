...
def loss_computation(logits_list, labels, losses, nclass, edges=None):
    ...
    for i in range(len(logits_list)):
        ...
        if edges is not None and i != 0:
            loss_list.append(losses['coef'][i] * loss_i(logits, edges))
        elif loss_i.__class__.__name__ in ('BCELoss'):
            label_app = _get_batch_label_vector(labels, nclass)
            from paddle.nn import BCELoss
            bce_loss = BCELoss()
            loss_list.append(losses['coef'][i] * bce_loss(logits, label_app))
        else:
...
def _get_batch_label_vector(target, nclass):
    ...
    tvect = paddle.fluid.layers.zeros([batch, nclass], dtype='float32')
    for i in range(batch):
        hist = np.histogram(target[i].numpy(),
                            bins=nclass, range=(0, nclass-1))[0]
        vect = hist
        if np.max(vect) > 0:
            vect = (vect - np.min(vect)) / (np.max(vect) - np.min(vect))
            vect = vect / np.sum(vect)
        tvect[i] = paddle.to_tensor(vect)
    return tvect
def train(model,
          train_dataset,
          val_dataset=None,
