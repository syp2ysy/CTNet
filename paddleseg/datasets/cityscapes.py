...
class Cityscapes(Dataset):
    ...
    def __init__(self, transforms, dataset_root, mode='train', edge=False):
        ...
        if mode=='trainval':
            label_train = sorted(
                glob.glob(os.path.join(label_dir, 'train', '*',
                             '*_gtFine_labelTrainIds.png')))
            label_val = sorted(
                glob.glob(os.path.join(label_dir, 'val', '*',
                             '*_gtFine_labelTrainIds.png')))
            img_train = sorted(
                glob.glob(os.path.join(img_dir, 'train', '*', '*_leftImg8bit.png')))
            img_val = sorted(
                glob.glob(os.path.join(img_dir, 'val', '*', '*_leftImg8bit.png')))	
            label_files = label_train + label_val
            img_files = img_train + img_val
        else:
            label_files = sorted(
