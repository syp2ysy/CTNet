...
class ResNet_vd(nn.Layer):
    ...
    def __init__(self,
        ...
        if output_stride == 8:
            ...
            dilation_dict= None
            self.jpu= True
            self.JPU = JPU(in_channels=[512,1024,2048], width=512)
    ...
    def forward(self, inputs):
        ...
        for stage in self.stage_list:
            ...
            feat_list.append(y)
        if self.jpu == True:
            feat_list = self.JPU(feat_list[0],feat_list[1],feat_list[2],feat_list[3])
        return feat_list
...
class SeparableConv2d(nn.Layer):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, padding=1, dilation=1, bias=False, ):
        ...
        self.bn = layers.SyncBatchNorm(inplanes)
...
class JPU(nn.Layer):
    ...
    def forward(self, *inputs):
        ...
        shape_t = feats[-1].shape
        feats[-2] = F.interpolate(feats[-2], [shape_t[-2],shape_t[-1]], mode='bilinear', align_corners=False)
        feats[-3] = F.interpolate(feats[-3], [shape_t[-2],shape_t[-1]], mode='bilinear', align_corners=False)
